    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Based on the generated ontology, documents are automatically chunked and fed to Zep to build a knowledge graph. Entities and relations are extracted, and temporal memory with community summaries is formed.
          </p>
      ...
      <div class="step-card" :class="{ 'active': currentPhase === 2, 'completed': currentPhase >= 2 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Graph construction is complete. Proceed to the next step to set up the simulation environment.</p>
          <button
            class="action-btn"
...
const handleEnterEnvSetup = async () => {
  ...
  try {
    ...
    if (res.success && res.data?.simulation_id) {
      ...
      showToast('시뮬레이션 생성에 실패했습니다: ' + (res.error || '알 수 없는 오류'), 'error')
    }
  } catch (err) {
    console.error('Error creating simulation:', err)
    showToast('시뮬레이션 생성 중 오류가 발생했습니다: ' + err.message, 'error')
  } finally {
