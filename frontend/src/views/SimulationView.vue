 ...
 * Called when the user returns from Step 3 to Step 2, indicating intent to exit the simulation.
 */
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      ...
      try {
        ...
        if (closeRes.success) {
          ...
          // Fall back to force stop if graceful shutdown fails
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(`Exception closing simulation environment: ${closeErr.message}`)
        // Fall back to force stop on exception
        await forceStopSimulation()
      }
    } else {
...
const forceStopSimulation = async () => {
  try {
    ...
    addLog(`Exception in force stop simulation: ${err.message}`)
  }
}
...
onMounted(async () => {
  ...
  // Check and stop any running simulation (triggered when user returns from Step 3)
