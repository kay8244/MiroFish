...
def generate_report():
    ...
    GET /api/report/generate/status 를 통해 진행 상황 조회 가능

    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",    // 필수, 시뮬레이션 ID
            "force_regenerate": false        // 선택적, 강제 재생성
        }
    ...
    try:
        ...
        # 프론트엔드에 즉시 반환할 수 있도록 report_id 미리 생성
        import uuid
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": "보고서 생성 태스크가 시작되었습니다. /api/report/generate/status 를 통해 진행 상황을 확인하세요",
...
def get_generate_status():
    ...
    요청 (JSON):
        {
            "task_id": "task_xxxx",         // 선택적, generate가 반환한 task_id
            "simulation_id": "sim_xxxx"     // 선택적, 시뮬레이션 ID
        }
...
def get_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": f"보고서가 존재하지 않음: {report_id}"
            }), 404
...
def list_reports():
    ...
    Query 파라미터:
        simulation_id: 시뮬레이션 ID로 필터링 (선택적)
        limit: 반환 수 제한 (기본값 50)
...
def chat_with_report_agent():
    ...
    Report Agent는 대화 중 검색 도구를 자율적으로 호출하여 질문에 답변 가능

    요청 (JSON):
        {
            "simulation_id": "sim_xxxx",        // 필수, 시뮬레이션 ID
            "message": "여론 동향을 설명해주세요",    // 필수, 사용자 메시지
            "chat_history": [                   // 선택적, 대화 이력
                {"role": "user", "content": "..."},
...
def get_report_progress(report_id: str):
    ...
    반환:
        {
            ...
            "data": {
                ...
                "completed_sections": ["요약", "시뮬레이션 배경"],
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": f"보고서가 존재하지 않거나 진행 상황 정보를 사용할 수 없음: {report_id}"
            }), 404
...
def get_report_sections(report_id: str):
    ...
    프론트엔드가 이 인터페이스를 폴링하여 생성된 섹션 내용을 가져올 수 있으며,
    전체 보고서 완료를 기다릴 필요 없음
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        if not os.path.exists(section_path):
            return jsonify({
                "success": False,
                "error": f"섹션이 존재하지 않음: section_{section_index:02d}.md"
            }), 404
...
def check_report_status(simulation_id: str):
    """
    시뮬레이션에 보고서가 있는지, 보고서 상태를 확인

    프론트엔드가 Interview 기능 잠금 해제 여부를 판단하는 데 사용
...
def get_agent_log(report_id: str):
    ...
    보고서 생성 과정의 각 단계 동작을 실시간으로 가져옴:
    - 보고서 시작, 계획 시작/완료
    - 각 섹션의 시작, 도구 호출, LLM 응답, 완료
    - 보고서 완료 또는 실패

    Query 파라미터:
        from_line: 몇 번째 줄부터 읽을지 (선택적, 기본값 0, 증분 조회용)
...
def get_console_log(report_id: str):
    ...
    보고서 생성 과정의 콘솔 출력 (INFO, WARNING 등)을 실시간으로 가져옴.
    agent-log 인터페이스가 반환하는 구조화된 JSON 로그와 달리,
