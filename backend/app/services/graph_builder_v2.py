"""Graphiti 기반 그래프 빌더 (Phase 2 교체품).

기존 graph_builder.GraphBuilderService의 외부 API를 유지하면서 내부 구현을
Zep Cloud SDK → Graphiti(Neo4j)로 바꾼다.
...
adapter(pipeline_adapters.graph_adapter)는 이 파일의 GraphBuilderServiceV2로
전환 가능. 기존 graph_builder.py는 Phase 6에서 삭제.
"""
...
logger = get_logger('mirofish.graph_builder_v2')
...
class GraphBuilderServiceV2:
    """Graphiti + Neo4j 기반 그래프 빌더. graph_builder.GraphBuilderService 대체."""
